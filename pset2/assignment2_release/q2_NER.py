...
class NERModel(LanguageModel):
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer") as scope:
        W = tf.get_variable("weights", shape=[self.config.window_size * self.config.embed_size, self.config.hidden_size], initializer=xavier_initializer)
        b1 = tf.get_variable(name="biases", shape=[self.config.hidden_size, ], initializer=xavier_initializer)
    hidden = tf.tanh(tf.matmul(window, W) + b1)
    hidden_dropout = tf.nn.dropout(hidden, self.dropout_placeholder)

    with tf.variable_scope("Softmax") as scope:
        U = tf.get_variable("weights", shape=[self.config.hidden_size, self.config.label_size], initializer=xavier_initializer)
        b2 = tf.get_variable(name="biases", shape=[self.config.label_size, ], initializer=xavier_initializer)
    output = tf.matmul(hidden_dropout, U) + b2
  ...
  def add_loss_op(self, y):
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(y, self.labels_placeholder)) + tf.constant(0.5 * self.config.l2) * (tf.reduce_sum(W*W) + tf.reduce_sum(U*U))
    ### END YOUR CODE
    return loss
