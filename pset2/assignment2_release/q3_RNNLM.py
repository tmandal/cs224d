...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      embed_vectors = tf.nn.embedding_lookup(L, self.input_placeholder)
      # inputs - current shape = (batch_size, num_steps, embed_size)
      split_embed_vectors = tf.split(1, self.config.num_steps, embed_vectors)
      inputs = [tf.squeeze(embed_vector, squeeze_dims=[1]) for embed_vector in split_embed_vectors]
  ...
  def add_projection(self, rnn_outputs):
    ...
    """
    ### YOUR CODE HERE
    U = tf.get_variable("OutputLayer_weights", [self.config.hidden_size, len(self.vocab)])
    b2 = tf.get_variable("OutputLayer_biases", [len(self.vocab), ])
    outputs = [None] * len(rnn_outputs)
    for step in xrange(len(rnn_outputs)):
        outputs[step] = tf.matmul(rnn_outputs[step], U) + b2
    ### END YOUR CODE
    return outputs
  ...
  def add_loss_op(self, output):
    ...
    loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(output, tf.reshape(self.labels_placeholder, [-1, ])))
    ### END YOUR CODE
    return loss
  ...
  def add_model(self, inputs):
    ...
    H = tf.get_variable("HiddenLayer_hidden_weights", [self.config.hidden_size, self.config.hidden_size])
    I = tf.get_variable("HiddenLayer_input_weights", [self.config.embed_size, self.config.hidden_size])
    b1 = tf.get_variable("HiddenLayer_biases", [self.config.hidden_size, ])
    rnn_inputs = [None] * len(inputs)
    hidden = [None] * (len(inputs) + 1)
    rnn_outputs = [None] * len(inputs)
    hidden[0] = self.initial_state
    for step in xrange(len(inputs)):
        rnn_inputs[step] = tf.nn.dropout(inputs[step], self.dropout_placeholder)
        hidden[step+1] = tf.nn.sigmoid(tf.matmul(hidden[step], H) + tf.matmul(rnn_inputs[step], I) + b1)
        rnn_outputs[step] = tf.nn.dropout(hidden[step+1], self.dropout_placeholder)
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in xrange(stop_length):
    ### YOUR CODE HERE
    x = np.array(tokens[i]).reshape((1, 1))
    feed = {model.input_placeholder: x,
            model.labels_placeholder: x,    # dont care really
            model.initial_state: state,
            model.dropout_placeholder: 1.0}
    outs = session.run([model.predictions[-1], model.final_state], feed_dict=feed)
    state = outs.pop()
    y_pred = outs[0]
    if i < len(tokens) - 1:
      continue
