...
def softmax(x):
  ...
  ### YOUR CODE HERE
  max = tf.reduce_max(x, reduction_indices=1)
  max = tf.reshape(max, [-1, 1])
  exp_x_adj = tf.exp(x - max)
  exp_x_adj_sum = tf.reduce_sum(exp_x_adj, reduction_indices=1)
  out = exp_x_adj / tf.reshape(exp_x_adj_sum, [-1, 1])
  ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
  ...
  """
  ### YOUR CODE HERE
  cost = - tf.to_float(y) * tf.log(yhat)
  out = tf.reduce_sum(cost) 
  ### END YOUR CODE
  return out
