...
class SoftmaxModel(Model):
  ...
  def add_training_op(self, loss):
    ...
    train_op = gd_optimizer.minimize(loss)
    ### END YOUR CODE
    return train_op
  ...
  def add_model(self, input_data):
    ...
    with tf.variable_scope("softmax") as scope:
        W = tf.get_variable("weights", initializer=tf.zeros([self.config.n_features, self.config.n_classes]))
        b = tf.get_variable(name="biases", initializer=tf.zeros([self.config.n_classes]))
