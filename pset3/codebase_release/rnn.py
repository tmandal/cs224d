...
class RNN_Model():
    ...
    def load_data(self):
        ...
        #self.train_data, self.dev_data, self.test_data = tr.simplified_data(350, 50, 100)
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ### YOUR CODE HERE
            curr_node_tensor = tf.reshape(tf.gather(embedding, self.vocab.encode(node.word)), [1, -1])
            #print "Leaf node word=", node.word, " node=", node, " tensor=", current_node_tensor
            ### END YOUR CODE
        else:
            node_tensors.update(self.add_model(node.left))
            node_tensors.update(self.add_model(node.right))
            ### YOUR CODE HERE
            #print "Left  node word=", node.left.word, " node=", node.left, " tensor=", node_tensors[node.left]
            #print "Right node word=", node.right.word, " node=", node.right, " tensor=", node_tensors[node.right]
            h = tf.concat(1, [node_tensors[node.left], node_tensors[node.right]])
            z = tf.matmul(h, W1) + b1
            curr_node_tensor = tf.nn.relu(z)
            ### END YOUR CODE
    ...
    def loss(self, logits, labels):
        ...
        with tf.variable_scope('Projection', reuse=True):
            U = tf.get_variable("U", [self.config.embed_size, self.config.label_size])
        loss = tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(logits, labels)) + tf.constant(self.config.l2 * 0.5) * (tf.nn.l2_loss(W1) + tf.nn.l2_loss(U))
        # END YOUR CODE
        return loss
    ...
    def predictions(self, y):
        ...
        predictions = None
        # YOUR CODE HERE
        predictions = tf.argmax(tf.nn.softmax(y), 1)
        # END YOUR CODE
        return predictions
